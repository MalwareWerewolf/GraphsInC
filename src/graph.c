...
Graph* create_graph(Edge edges[], int num_edges, int initial_vertices) {
  ...
  for (int i = 0; i < num_edges; i++) {
    ...
    if (source >= g->vertex_count || dest >= g->vertex_count) continue;
...
int graph_add_vertex(Graph *g) {
  ...
  if (g->vertex_count >= g->capacity) {
    ...
    AdjNode **new_lists = realloc(g->adj_lists, new_capacity * sizeof(AdjNode *));
    if (!new_lists) return -1;
...
int graph_remove_vertex(Graph *g, int v) {
  if (!g || v < 0 || v >= g->vertex_count) return -1;
